...
> Measured on Apple M2 Pro, macOS, Go 1.25, Docker Desktop. Results vary by hardware and system load.

### Internal Micro-Benchmarks

Core operations measured in isolation (no network overhead). Reproduce with `make bench`.

**Cache Store** (256 shards, `sync.RWMutex` per shard):
...
**Protocol** (parse + marshal per command):
...
| ------------------ | --------: | --------: | ------------- |
| SET 64B            |    143 us |    185 us | 1.30x faster  |
| SET 1KB            |    155 us |    196 us | 1.26x faster  |
| GET 64B (hit)      |    356 us |    506 us | 1.42x faster  |

tinycache outperforms memcached on single-connection SET and GET for small values. This is the goroutine-per-connection model with `bufio` beating libevent's async I/O overhead when there's no concurrency to multiplex.

**Caveats:** these benchmarks measure single-client, single-node throughput through Docker Desktop networking on macOS. Production performance will differ. Memcached is expected to scale better at very high connection counts due to its C/libevent architecture. Larger value sizes and mixed workloads show more variance; run `make bench-compare` on your target hardware.
